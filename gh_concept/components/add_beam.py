...
@hops.component(
    ...
    inputs=[
        ...
        hs.HopsString(
            "Beam name", "Name", "Name of beam as list", hs.HopsParamAccess.LIST, True
        ),
        hs.HopsNumber(
            "Beam thickness",
            "Thickness",
        ...
        hs.HopsNumber(
            "Beam width", "Width", "Beam width as list", hs.HopsParamAccess.LIST
        ),
        hs.HopsNumber(
            "Top of concrete",
            "TopLevel",
...
def add_beam(
    ...
    width: list[float],
    top_of_concrete: list[float],
    priority: list[int],
    behaviour: list[str],
    material: list[str],
    mesh_as_slab: list[float],
):
    if push:
        logger = []
        ...
        for i, beam_line in enumerate(line):
            ...
            if valid:
                ...
                valid_beams.append(
                    {
                        "line_segments": concept_geo.lines_from_polyline(beam_line),
                        ...
                        "top_of_concrete": top_of_concrete[i]
                        if i < len(top_of_concrete)
                        else None,
                        ...
                        "mesh_as_slab": mesh_as_slab[i]
                        if i < len(mesh_as_slab)
                        else None,
                    }
                )
        if valid_beams:
            ...
            logger.append("No valid beams provided.")
        return logger
